...
async function getHabit(id: string): Promise<Habit | null> {
  try {
    ...
    const data: ApiResponse<{ habit: Habit }> = await response.json();
...
async function getHabitPerformance(id: string): Promise<HabitPerformance | null> {
  try {
    const cookieStore = await cookies();
    ...
    if (!response.ok) return null;
    const data: ApiResponse<{ performance: HabitPerformance }> = await response.json();
...
async function getTodayEntry(habitId: string): Promise<HabitEntry | null> {
  try {
    const today = getToday();
    const cookieStore = await cookies();
    const response = await fetch(
      `${API_BASE_URL}/habits/${habitId}/history?startDate=${today}&endDate=${today}&limit=1`,
      {
        credentials: 'include',
        headers: { Cookie: cookieStore.toString() },
      }
    ...
    if (!response.ok) return null;
    const data: ApiResponse<HabitEntry[]> = await response.json();
...
export default async function HabitDetailPage({ params }: PageProps) {
  ...
  const [habit, performance, todayEntry] = await Promise.all([
    getHabit(id),
    getHabitPerformance(id),
    getTodayEntry(id),
  ]);
